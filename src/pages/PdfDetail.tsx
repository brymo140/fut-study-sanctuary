...
const PdfDetail = () => {
  ...
  const load = async () => {
    ...
    const chaptersData = ((ch as Chapter[]) || []) as Chapter[];
    ...
    try {
      ...
      const currentChapterIds = new Set(chaptersData.map((c) => c.id));
 ...
 const openRead = (ch: Chapter) => {
  ...
  const reportFile = async () => {
    ...
    await supabase.from("reports").insert({
      user_id: user.id,
      pdf_id: id,
      reason: "Bad file reported by user"
    });
  ...
  const submitRating = async () => {
    ...
    const { error } = await supabase
      .from("ratings")
      .upsert(payload, { onConflict: "user_id,pdf_id" });
    if (error) {
      ...
      <div className="relative bg-gradient-cover px-4 pt-4 pb-8 rounded-b-3xl">
        ...
        <div className="flex flex-col items-center mt-2">
          ...
          <p className="text-sm text-white/80 mt-1">
            {pdf.course_code} · {pdf.faculty || pdf.department || "General"}
          </p>
          ...
          <div className="surface-card p-3">
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <span className="font-semibold text-primary">
                {unlockedCount} of {chapters.length} unlocked
              </span>
            </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium line-clamp-1">
                          <span className="text-muted-foreground mr-1">M{ch.chapter_number}.</span>
                          {ch.title}
                        </p>
                ...
                <div key={r.id} className="surface-card p-3">
                  <div className="flex items-center gap-1 mb-1.5">
                    ...
                    <span className="text-[10px] text-muted-foreground ml-2">
                      {reviewUsers[r.user_id] || "Student"}
                    </span>
                  </div>
                  {r.review_text && (
                    <p className="text-xs text-foreground/90">{r.review_text}</p>
                  )}
                </div>
              ))}
            </div>
