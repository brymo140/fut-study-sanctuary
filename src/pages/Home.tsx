...
const Home = ({ autoOpenNotifications = false }: { autoOpenNotifications?: boolean }) => {
  ...
  useEffect(() => {
    if (user?.id) maybeShowStudyReminder();
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const chQ = supabase.from("youtube_channels").select("id,channel_name,channel_url,thumbnail_url,level")
  ...
  // Unread count — announcements only (no PDF notifications)
  const computeUnread = async () => {
    ...
    const [{ data: ann }, { data: reads }] = await Promise.all([
      supabase.from("announcements")
        .select("id,target_level,created_at,is_active")
        .gte("created_at", since).eq("is_active", true).limit(100),
  ...
  }, [user?.id, profile?.level, notifOpen]);

  // Realtime bump for new announcements only
  useEffect(() => {
  ...
  if (loading) {
    return (
      <div className="space-y-5">
        {[1, 2, 3].map(i => <div key={i} className="surface-card p-4 animate-pulse" />)}
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Single notification bell — announcements + alerts */}
          <button
            onClick={() => setNotifOpen(true)}
            aria-label="Notifications"
            className="relative h-9 w-9 rounded-full surface-card flex items-center justify-center hover:border-primary"
          >
      ...
      <Section title="Trending this week" subtitle="Most downloaded by your peers">
        {trending.length === 0 ? <EmptyHint text="No trending materials yet. Check back soon." /> : (
          <div className="flex gap-3 overflow-x-auto scrollbar-hide">
            {trending.map((p, i) => (
              <div key={p.id} className="relative">
                {i === 0 && <span className="absolute -top-1 -right-1 z-10 badge-purple">Trending</span>}
      ...
      >
        {channels.length === 0 ? <EmptyHint text="No channels yet." /> : (
          <div className="flex gap-3 overflow-x-auto scrollbar-hide">
            {channels.map((c) => (
              <button key={c.id} onClick={() => openYouTube(c.channel_url)} type="button"
      ...
      <Section title="Recently added" subtitle="Fresh uploads from class reps">
        {recent.length === 0 ? <EmptyHint text="Nothing here yet — uploads will show up as they arrive." /> : (
